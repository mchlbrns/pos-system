 ...
 * @description Seed data for demo: sample businesses, users, categories, products,
 * and default print templates for waterstation, laundry, and motor repair.
 */
...
/**
 * Seeds the database with demo data. Safe to run multiple times —
...
function seedDatabase() {
  ...
  const seed = db.transaction(() => {
    ...
    const biz1 = insertBiz.run(
      ...
      JSON.stringify({ vat_inclusive: true, currency: 'PHP' })
    );
    ...
    // ----------------------------------------------------------------
    // USERS (password: "admin123" for all demo users)
    // ----------------------------------------------------------------
...
// Allow running directly: node src/database/seeds.js
